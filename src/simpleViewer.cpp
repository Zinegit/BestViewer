...
void Viewer::drawSurfaces()
{
	...
	glDrawElements(GL_TRIANGLES, m_nb_gindices, GL_UNSIGNED_INT, NULL);
	glDisableVertexAttribArray(0);
}
...
//        }
	...
	else if (outside_camera)
	{
		...
		qglviewer::Vec out_dir = outside_camera->viewDirection();
		qglviewer::Vec out_pos = outside_camera->position();

		glLineWidth(2.5);
		glColor3f(0.0, 1.0, 0.0);
		glBegin(GL_LINES);
			glVertex3f(out_dir.x, out_dir.y, out_dir.z);
