...
std::vector<int> updateIndex(std::vector<bool>& triangles_to_show, std::vector<int>& indices)
{
	...
	for (int i = 0; i < triangles_to_show.size(); i++)
	{
		...
		{
			new_index.push_back(indices[3 * i]);
			new_index.push_back(indices[3 * i + 1]);
			new_index.push_back(indices[3 * i + 2]);
		}
	}
	return new_index;
}
...
//	{
//		new_vertices.push_back(3 * indices[i]);
//		new_vertices.push_back(3 * indices[i]+1);
//		new_vertices.push_back(3 * indices[i]+2);
//	}
//}
